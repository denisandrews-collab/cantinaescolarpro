...
# This script:
# - fetches the remote branch fix/posview-stubs and checks it out locally
# - creates components/PosView.tsx, components/Receipt.tsx, services/geminiService.ts, services/itauService.ts
# - commits and pushes the changes to origin/fix/posview-stubs
#
# Usage: run from the repository root:
#   bash create_posview_stubs.sh
#
# Review the files before pushing if you prefer to paste them manually.

echo "Fetching remote branches..."
git fetch origin

# Try to checkout the remote branch; if it doesn't exist locally, create tracking branch
if git rev-parse --verify --quiet refs/heads/fix/posview-stubs >/dev/null; then
  git checkout fix/posview-stubs
else
  # If remote branch exists, create local tracking branch; otherwise create new local branch
  if git ls-remote --exit-code --heads origin fix/posview-stubs >/dev/null 2>&1; then
    git checkout -b fix/posview-stubs origin/fix/posview-stubs
  else
    git checkout -b fix/posview-stubs
  fi
fi

mkdir -p components services

cat > components/PosView.tsx <<'EOF'
...
export const PosView: React.FC<PosViewProps> = ({
  ...
  const studentInputRef = useRef<HTMLDivElement | null>(null);
  ...
  const filteredStudents = useMemo(() => {
    ...
    return students.filter(s => (s.isActive !== false) && (s.name.toLowerCase().includes(q) || (s.grade && s.grade.toLowerCase().includes(q)) || (s.code && s.code.toLowerCase().includes(q))));
  ...
  const checkOverdueStatus = (student: Student) => {
    ...
    for (const entry of history) {
      if (entry.balanceAfter !== undefined && entry.balanceAfter < 0) {
        if (debtStartDate === null) debtStartDate = new Date(entry.date);
  ...
  const addToCart = (product: Product) => {
    ...
    if (enforceStock && product.stock !== undefined && product.stock <= 0) { alert('Produto sem estoque!'); return; }
    setCart(prev => {
      const existing = prev.find(item => item.id === product.id);
      if (enforceStock && existing && product.stock !== undefined && existing.quantity >= product.stock) { alert('Estoque insuficiente!'); return prev; }
  ...
  const handleSaveCashEntry = () => {
    if (onAddCashEntry) {
      const value = parseFloat(cashAmount) || 0;
      onAddCashEntry({
        id: Date.now().toString(),
        date: new Date(),
        type: cashType,
        amount: value,
        value: cashType === 'IN' ? value : -value,
  ...
  const initiateStudentAccountPayment = () => {
    ...
    } else { alert('Selecione um aluno primeiro.'); }
  };
  ...
  const calculateChange = () => { const received = parseFloat(cashReceived); if (isNaN(received)) return 0; return received - cartTotal; };
  const handleQuickMoney = (amount: number) => { setCashReceived(amount.toString()); };
  const getStockBadgeClass = (stock: number) => { if (stock === 0) return 'bg-red-200 text-red-800'; if (stock <= 10) return 'bg-yellow-200 text-yellow-800'; return 'bg-green-200 text-green-800'; };
  ...
  return (
    <div className="flex h-full overflow-hidden bg-gray-100">
              ...
              return (
                <button key={product.id} onClick={() => addToCart(product)} disabled={isStockBlocked} className={`flex flex-col bg-white rounded-xl shadow-sm border transition-all text-left overflow-hidden group h-full relative ${!isStockBlocked ? 'hover:shadow-md hover:border-green-300 border-gray-200' : 'opacity-60 cursor-not-allowed border-gray-200'}`}>
                  ...
                  <div className={`h-32 w-full flex items-center justify-center relative overflow-hidden ${!isStockBlocked ? 'bg-gray-50' : 'bg-gray-100'}`}>
                    {product.image ? <img src={product.image} className="w-full h-full object-cover" alt={product.name} /> : <div className="text-gray-300 transform scale-150">{CategoryIcons[product.category]}</div>}
                  </div>
      ...
      <div className="w-96 bg-white border-l border-gray-200 flex flex-col shrink-0 shadow-2xl z-10">
        <div className="p-4 bg-gray-50 border-b border-gray-200 relative" ref={studentInputRef}>
          ...
          {selectedStudent ? (
            ...
            settings.paymentMethods.studentAccount ? (
              <div className="relative">
                <input type="text" placeholder="Selecionar Aluno (Nome/Código)..." className="w-full py-3 px-4 border-2 border-dashed border-gray-300 rounded-lg text-gray-700 placeholder-gray-500 focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500 bg-white" value={studentSearchQuery} onChange={(e) => { setStudentSearchQuery(e.target.value); setIsStudentDropdownOpen(true); }} onFocus={() => setIsStudentDropdownOpen(true)} />
                {isStudentDropdownOpen && studentSearchQuery && (
                  <div className="absolute top-full left-0 w-full mt-1 bg-white border border-gray-200 shadow-xl rounded-lg max-h-60 overflow-y-auto z-50">
                    {filteredStudents.length > 0 ? (
                      filteredStudents.map(student => (<button key={student.id} onMouseDown={() => handleSelectStudent(student)} className="w-full text-left p-3 hover:bg-gray-50 border-b border-gray-50 last:border-0"><p className="font-bold text-gray-800 text-sm flex items-center gap-1">{student.name}{student.isStaff && <span className="bg-purple-100 text-purple-700 text-[10px] px-1 rounded uppercase">Func</span>}</p><div className="flex justify-between items-center text-xs mt-0.5"><span className="text-gray-500">{student.grade}</span><span className={student.balance < 0 ? 'text-red-500 font-bold' : 'text-green-600'}>{student.balance < 0 ? `Dev: R$ ${Math.abs(student.balance).toFixed(2)}` : `R$ ${student.balance.toFixed(2)}`}</span></div></button>))
                    ) : (<div className="p-3 text-center"><p className="text-sm text-gray-500 mb-2">Aluno não encontrado.</p><button onMouseDown={() => { setIsStudentDropdownOpen(false); onRequestQuickRegister(); }} className="text-xs font-bold text-brand-600 hover:underline">+ Cadastrar Novo</button></div>)}
                  </div>
                )}
              </div>
            ) : (<div className="p-3 bg-gray-100 rounded text-center text-xs text-gray-500">Conta Aluno OFF</div>)
          )}
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
          {cart.map(item => (
            <div key={item.id} className="flex justify-between items-center bg-gray-50 p-2 rounded">
              <div><p className="font-bold text-sm">{item.name}</p><p className="text-xs text-gray-500">{item.quantity}x R$ {item.price.toFixed(2)}</p>{item.notes && <p className="text-[10px] text-orange-600 italic">Obs: {item.notes}</p>}</div>
              <div className="flex items-center gap-2"><p className="font-bold">R$ {(item.quantity * item.price).toFixed(2)}</p><button onClick={() => removeFromCart(item.id)} className="text-red-500">x</button></div>
            </div>
          ))}
        </div>
        ...
        <div className="bg-white border-t p-6 shadow-lg">
          ...
          <div className="grid grid-cols-2 gap-2 mb-2">
            {settings.paymentMethods.money && <button disabled={cart.length === 0 || isProcessing} onClick={() => { setShowCashModal(true); setCashReceived(''); }} className="py-3 bg-green-600 text-white font-bold rounded-lg hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed">Dinheiro</button>}
            {settings.paymentMethods.pix && <button disabled={cart.length === 0 || isProcessing} onClick={initiatePixPayment} className="py-3 bg-orange-500 text-white font-bold rounded-lg hover:bg-orange-600 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-1"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14" /><path d="m12 5 7 7-7 7" /></svg>Pix</button>}
            {settings.paymentMethods.creditCard && <button disabled={cart.length === 0 || isProcessing} onClick={() => processTransaction('CREDIT')} className="py-3 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed">Crédito</button>}
            {settings.paymentMethods.debitCard && <button disabled={cart.length === 0 || isProcessing} onClick={() => processTransaction('DEBIT')} className="py-3 bg-cyan-600 text-white font-bold rounded-lg hover:bg-cyan-700 disabled:opacity-50 disabled:cursor-not-allowed">Débito</button>}
            {settings.paymentMethods.studentAccount && <button disabled={cart.length === 0 || isProcessing} onClick={initiateStudentAccountPayment} className="col-span-2 py-3 bg-brand-600 text-white font-bold rounded-lg hover:bg-brand-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect width="20" height="14" x="2" y="5" rx="2" /><line x1="2" x2="22" y1="10" y2="10" /></svg>{selectedStudent ? (overdueStatus.isOverdue ? 'Bloqueado' : 'Conta Aluno') : 'Conta Aluno (Selecionar)'}</button>}
          </div>
          {cart.length > 0 && <button onClick={() => setCart([])} className="w-full mt-2 py-2 text-xs font-bold text-red-500 hover:text-red-700 hover:bg-red-50 rounded">Cancelar Venda (Limpar)</button>}
        </div>
      </div>
      ...
      {showCashModal && (
        <div className="fixed inset-0 bg-black/60 z-40 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl p-6">
            ...
            <div className="flex flex-wrap gap-2 justify-center mb-4">{[2, 5, 10, 20, 50, 100].map(val => <button key={val} onClick={() => handleQuickMoney(val)} className="bg-green-100 text-green-800 px-3 py-1 rounded font-bold text-sm hover:bg-green-200">R$ {val}</button>)}</div>
      ...
      {showPixModal && (
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
          <div className="bg-white p-6 rounded shadow-lg text-center">
            <h3 className="font-bold text-lg mb-4">Pagamento Pix</h3>
            {isGeneratingPix ? <p>Gerando QR Code...</p> : pixData ? (
              <div>
                <img src={pixData.qrCodeImage} className="mx-auto w-48 h-48" alt="Pix QR" />
                <p className="text-xs bg-gray-100 p-2 mt-2 font-mono break-all">{pixData.pixCopiaCola}</p>
                <button onClick={() => processTransaction('PIX')} className="mt-4 w-full bg-green-600 text-white py-2 rounded font-bold">Confirmar Pagamento</button>
              </div>
            ) : <p>Erro.</p>}
            <button onClick={() => setShowPixModal(false)} className="mt-2 text-gray-500">Cancelar</button>
          </div>
        </div>
      )}
      ...
      {showCashDrawerModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white p-6 rounded max-w-sm w-full">
            <h3 className="font-bold text-lg mb-4">Gestão de Caixa</h3>
            <div className="flex gap-2 mb-4">
              <button onClick={() => setCashType('IN')} className={`flex-1 py-2 rounded ${cashType === 'IN' ? 'bg-green-100 text-green-700' : 'bg-gray-100'}`}>Suprimento</button>
              <button onClick={() => setCashType('OUT')} className={`flex-1 py-2 rounded ${cashType === 'OUT' ? 'bg-red-100 text-red-700' : 'bg-gray-100'}`}>Sangria</button>
            </div>
            <input type="number" placeholder="Valor" value={cashAmount} onChange={e => setCashAmount(e.target.value)} className="w-full border p-2 mb-2 rounded" />
            <input type="text" placeholder="Motivo" value={cashDesc} onChange={e => setCashDesc(e.target.value)} className="w-full border p-2 mb-4 rounded" />
            <div className="flex gap-2">
              <button onClick={() => setShowCashDrawerModal(false)} className="flex-1 py-2 bg-gray-200 rounded">Cancelar</button>
              <button onClick={handleSaveCashEntry} className="flex-1 py-2 bg-blue-600 text-white rounded">Salvar</button>
            </div>
          </div>
        </div>
      )}
...
cat > components/Receipt.tsx <<'EOF'
import React from 'react';
import { Transaction, SystemSettings } from '../types';

export const Receipt: React.FC<{ transaction: Transaction; settings: SystemSettings; onClose: () => void }> = ({ transaction, settings, onClose }) => {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div className="bg-white max-w-md w-full rounded-lg shadow-lg p-4">
        <div className="flex justify-between items-center mb-4">
          <h3 className="font-bold">Recibo</h3>
          <button onClick={onClose} className="text-sm text-gray-600 hover:underline">Fechar</button>
        </div>

        <div className="text-xs font-mono bg-gray-50 p-3 rounded mb-3 overflow-auto" style={{ maxHeight: 240 }}>
          <pre className="whitespace-pre-wrap break-words">{JSON.stringify(transaction, null, 2)}</pre>
        </div>

        <div className="flex justify-between items-center">
          <div className="text-xs text-gray-500">
            <div>{settings.schoolName}</div>
            <div className="mt-1">Total: R$ {transaction.total?.toFixed?.(2) ?? '0.00'}</div>
          </div>
          <div className="flex gap-2">
            <button
            ...
            <button onClick={onClose} className="px-3 py-1 bg-brand-600 text-white rounded text-sm hover:bg-brand-700">OK</button>
          </div>
        </div>
      </div>
    </div>
...
// Stub for generateReceiptMessage used by PosView
// Replace with real integration when available.

import { CartItem } from '../types';

export async function generateReceiptMessage(customerName: string, items: CartItem[]): Promise<string> {
  // Minimal async stub: returns a simple summary.
  return new Promise(resolve => {
    const lines = items.map(i => `${i.quantity}x ${i.name} - R$ ${(i.price * i.quantity).toFixed(2)}`);
    const text = `Recibo para ${customerName}\n\n${lines.join('\n')}\n\nTotal: R$ ${items.reduce((s, it) => s + it.price * it.quantity, 0).toFixed(2)}\n\nObrigado!`;
    setTimeout(() => resolve(text), 300); // simulate small latency
  });
}

export default generateReceiptMessage;
EOF

cat > services/itauService.ts <<'EOF'
...
git add components/PosView.tsx components/Receipt.tsx services/geminiService.ts services/itauService.ts

if git diff --staged --quiet; then
  echo "No changes to commit."
else
  git commit -m "fix: repair PosView and add stubs for Receipt and services"
  echo "Pushing branch fix/posview-stubs to origin..."
  git push --set-upstream origin fix/posview-stubs
fi

echo ""
echo "Done. Branch fix/posview-stubs has been updated with the files."
echo "Open a Pull Request on GitHub from fix/posview-stubs → main (or use 'gh pr create')."
EOF
