...
export const PosView: React.FC<PosViewProps> = ({ 
  ...
  const handleSelectStudent = (student: Student) => { setSelectedStudent(student); setShowStudentModal(false); setStudentSearchQuery(''); };
  ...
  return (
    <div className="flex h-full overflow-hidden bg-gray-100">
             ...
             <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {filteredProducts.map(product => {
                  ...
                  <button key={product.id} onClick={() => addToCart(product)} disabled={isStockBlocked} className={`flex flex-col bg-white rounded-xl shadow-sm border transition-all text-left overflow-hidden group h-full relative ${!isStockBlocked ? 'hover:shadow-md hover:border-green-300 border-gray-200' : 'opacity-60 cursor-not-allowed border-gray-200'}`}>
                    ...
                    <div className={`h-32 w-full flex items-center justify-center relative overflow-hidden ${!isStockBlocked ? 'bg-gray-50' : 'bg-gray-100'}`}>{product.image ? <img src={product.image} className="w-full h-full object-cover" /> : <div className="text-gray-300 transform scale-150">{CategoryIcons[product.category]}</div>}</div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
            {cart.map(item => (
                <div key={item.id} className="flex justify-between items-center bg-gray-50 p-2 rounded">
                    <div><p className="font-bold text-sm">{item.name}</p><p className="text-xs text-gray-500">{item.quantity}x R$ {item.price.toFixed(2)}</p></div>
      ...
      {/* MODALS */}
      {showStudentModal && ( <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4"> <div className="bg-white p-4 rounded w-full max-w-md h-96 overflow-y-auto"> <input autoFocus type="text" placeholder="Buscar..." className="w-full border p-2 mb-2" onChange={e => setStudentSearchQuery(e.target.value)} /> {filteredStudents.map(s => <div key={s.id} onClick={() => handleSelectStudent(s)} className="p-2 hover:bg-gray-100 cursor-pointer border-b">{s.name}</div>)} <button onClick={() => setShowStudentModal(false)} className="mt-4 w-full bg-gray-200 p-2 rounded">Fechar</button> </div> </div> )}
      {showCashModal && ( 
          <div className="fixed inset-0 bg-black/60 z-40 flex items-center justify-center p-4 backdrop-blur-sm">
             <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl p-6">
                 <h3 className="text-xl font-black text-gray-800 mb-6 text-center">Dinheiro</h3>
                 <div className="mb-4 text-center"><p className="text-4xl font-black">R$ {cartTotal.toFixed(2)}</p></div>
                 <input autoFocus type="number" step="0.01" value={cashReceived} onChange={(e) => setCashReceived(e.target.value)} className="w-full p-3 text-xl font-bold border-2 rounded mb-4 focus:ring-2 focus:ring-green-500 outline-none" placeholder="Valor Recebido" />
                 {/* Quick Money Buttons */}
