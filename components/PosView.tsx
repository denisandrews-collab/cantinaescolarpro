...
export const PosView: React.FC<PosViewProps> = ({ 
  ...
  return (
    <div className="flex h-full overflow-hidden bg-gray-100">
      ...
      {showCashModal && ( <div className="fixed inset-0 bg-black/60 z-40 flex items-center justify-center p-4 backdrop-blur-sm"> <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl p-6"> <h3 className="text-xl font-black text-gray-800 mb-6 text-center">Pagamento em Dinheiro</h3> <div className="mb-4 text-center"><p className="text-4xl font-black">R$ {cartTotal.toFixed(2)}</p></div> <input autoFocus type="number" step="0.01" value={cashReceived} onChange={(e) => setCashReceived(e.target.value)} className="w-full p-3 text-xl font-bold border-2 rounded mb-4" placeholder="Valor Recebido" /> <div className="flex flex-wrap gap-2 justify-center mb-4">{[2,5,10,20,50,100].map(val => <button key={val} onClick={() => handleQuickMoney(val)} className="bg-green-100 text-green-800 px-3 py-1 rounded font-bold text-sm hover:bg-green-200">R$ {val}</button>)}</div> {parseFloat(cashReceived) >= cartTotal && <div className="bg-green-100 p-4 rounded mb-4 text-center"><p className="font-bold text-green-800">Troco: R$ {calculateChange().toFixed(2)}</p></div>} <div className="flex gap-2"> <button onClick={() => setShowCashModal(false)} className="flex-1 py-3 bg-gray-200 font-bold rounded">Cancelar</button> <button onClick={() => processTransaction('MONEY')} disabled={parseFloat(cashReceived) < cartTotal} className="flex-1 py-3 bg-green-600 text-white font-bold rounded disabled:opacity-50">Confirmar</button> </div> </div> </div> )}
